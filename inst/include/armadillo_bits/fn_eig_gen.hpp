...
eig_gen
  ...
  if( auxlib::crippled_lapack(expr) && (sig == 'b') )  { arma_conform_warn(1,  "eig_gen(): 'balance' option ignored due to linking with crippled lapack"); }
...
eig_gen
  ...
  if(status == false)
    {
    eigvals.soft_reset();
    arma_conform_warn(3, "eig_gen(): decomposition failed");
    }
