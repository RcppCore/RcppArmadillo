    ...
    */
    ...
    for (int j=0;j<nn;j++) {
        if (j==0) {
            ...
            M5[j] = const3 + const4*t[i] + const5* pow(t[i],2);
        } else if (j==3) {
            M5[j] = const6 + const7*t[i] + const8* pow(t[i],2) + const9* pow(t[i],3);
        } else if (j==4) {
            M5[j] = const10 + const11*t[i] + const12* pow(t[i],2) + const13* pow(t[i],3) + const14* pow(t[i],4);
        } else {
